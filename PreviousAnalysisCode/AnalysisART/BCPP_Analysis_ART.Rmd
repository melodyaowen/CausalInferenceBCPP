...
all_names_filtered <- bind_rows(names1, names2, names3) %>%
  ...
  dplyr::filter(colname %in% c("de_subj_idC", # Subject identifier
                               ...
                               # Death Outcomes
                               "endpoint_death", # Endpoint: participant died by study completion
                               "death_primary_cause", # Primary cause of death
                               "death_group_cause", # Grouped cause of death
                               "death_days", # Days from enrollment to death
                               ...
                               "prev_hiv_test_num", # Number of previous HIV tests
                               "Viral_load", # Viral load result
                               "arv_status_current", # Current ARV status
                               "partners_lifetime", # Total number of sexual partners in lifetime
                               "partners_12mos", # Number of sexual partners, past year
                               "sex_12mos", # Sexually active in last year
                               "sex_outside_12mos", # Sex outside community, past year
                               ...
                               ) # Incidence endpoint: seroconverted by study completion
  )
...
combinedDat <- full_join(dat1_renamed, dat2_renamed, by = "de_subj_idC") %>%
  ...
  dplyr::select(de_subj_idC, starts_with("random_arm_dat"), 
                ...
                contains("endpoint_coverage_onart"),
                contains("endpoint_coverage_vlsupp"),
                contains("seroconvert"),
                contains("endpoint_death"), contains("death_primary_cause"),
                contains("death_group_cause"),
                contains("death_days"),
                
                # Individual covariates
                contains("age_at_interview"), # Age at interview
                contains("length_residence"), # How long lived in this community
                contains("marital_status"), # Marital status
                contains("education"), # Highest education level
                contains("employment_status"), # employment status
                contains("prev_hiv_test_num"), # Number of previous HIV tests
                contains("Viral_load"), # Viral load result
                contains("arv_status_current"), # Current ARV status
                contains("partners_lifetime"), # Total number of sexual partners in lifetime
                contains("partners_12mos"), # Number of sexual partners, past year
                contains("sex_12mos"), # Sexually active in last year
                contains("sex_outside_12mos"), # Sex outside community, past year
                contains("condom_lastsex"), # Condom use during most recent sex
                contains("exchange_12mos"), # Transactional sex
                contains("overall_access"), # If I need medical care, I can see professional
                               
                               # Cluster covariates
                contains("prob_housing"), # Neighborhood problem: housing
                contains("prob_schools"), # Neighborhood problem: schools
                contains("prob_hiv"), # Neighborhood problem: HIV
                contains("prob_alcohol_drug"), # Neighborhood problem: alcohol/drugs
                contains("prob_healthcare") # Neighborhood problem: healthcare"
...
combinedDat <- full_join(dat1_renamed, dat2_renamed, by = "de_subj_idC") %>%
  ...
  mutate(random_arm = ifelse(!is.na(random_arm_dat1), random_arm_dat1,
                             ifelse(!is.na(random_arm_dat2), random_arm_dat2,
                                    random_arm_dat3))) %>%
  dplyr::select(-random_arm_dat1, -random_arm_dat2, -random_arm_dat3) %>%
  mutate(gender = ifelse(!is.na(gender_dat1), gender_dat1,
                         ifelse(!is.na(gender_dat2), gender_dat2,
                                gender_dat3))) %>%
  dplyr::select(-gender_dat1, -gender_dat2, -gender_dat3) %>%
  mutate(community = ifelse(!is.na(community_dat1), community_dat1,
                            ifelse(!is.na(community_dat2), community_dat2,
                                   community_dat3))) %>%
  dplyr::select(-community_dat1, -community_dat2, -community_dat3) %>%
  
  
  rename(subject_ID = de_subj_idC,
         hiv_status_current = hiv_status_current_dat1,
         endpoint_coverage_htc = endpoint_coverage_htc_dat1,
         endpoint_seroconvert = endpoint_seroconvert_dat1,
         endpoint_death = endpoint_death_dat1,
         death_primary_cause = death_primary_cause_dat1,
         death_group_cause = death_group_cause_dat1,
         death_days = death_days_dat1) %>%
  
  
  relocate(subject_ID, community, gender, random_arm, hiv_status_current) %>%
...
modelDat_full <- cleanDat %>%
  ...
  mutate(
  X1_ik_vmmc_y1 = case_when(
    endpoint_coverage_mc_dat1 == "Yes" &
      endpoint_coverage_mc != "Began study circumcised" ~ 1L,
    TRUE ~ 0L
  ),
  X1_ik_vmmc_y2 = case_when(
    endpoint_coverage_mc_dat2 == "Yes" &
      (endpoint_coverage_mc_dat1 == "No" | is.na(endpoint_coverage_mc_dat1)) ~ 1L,
    TRUE ~ 0L
  ),
  X1_ik_vmmc_y3 = case_when(
    X1_ik_vmmc == "Yes" &
      (X1_ik_vmmc_y1 == 0 & X1_ik_vmmc_y2 == 0) ~ 1L,
    TRUE ~ 0L
  )) %>%
  dplyr::select(-starts_with("endpoint_coverage_mc_dat"),
                #-starts_with("circumcision_days_dat"),
                -starts_with("circumcised_dat")) %>%
  # Clean ART data
  mutate(X3_ik_y1 = ifelse(endpoint_coverage_onart_dat1 == "Yes", 1, 0),
         X3_ik_y2 = ifelse(endpoint_coverage_onart_dat2 == "Yes", 1, 0),
         X3_ik_y3 = ifelse(endpoint_coverage_onart_dat3 == "Yes", 1, 0),
         vlsupp_ik_y1 = ifelse(endpoint_coverage_vlsupp_dat1 == "Yes", 1, 0),
         vlsupp_ik_y2 = ifelse(endpoint_coverage_vlsupp_dat2 == "Yes", 1, 0),
         vlsupp_ik_y3 = ifelse(endpoint_coverage_vlsupp_dat3 == "Yes", 1, 0)
    ) %>%
  dplyr::select(-starts_with("endpoint_coverage_onart_dat")) %>%
  dplyr::select(-starts_with("endpoint_coverage_vlsupp_dat")) %>%
  group_by(cluster_id) %>%
  ...
  # Count of all male VMMC in each cluster Year 1
  mutate(vmmc_count_y1 = sum(X1_ik_vmmc == "Yes" & X1_ik_vmmc_y1 == 1, na.rm = TRUE)) %>%
  # Count of all male VMMC in each cluster Year 2
  mutate(vmmc_count_y2 = sum(X1_ik_vmmc == "Yes" & (X1_ik_vmmc_y2 == 1 | X1_ik_vmmc_y1 == 1), na.rm = TRUE)) %>%
  # Count of all male VMMC in each cluster Year 3
  mutate(vmmc_count_y3 = sum(X1_ik_vmmc == "Yes" & (X1_ik_vmmc_y3 == 1 | X1_ik_vmmc_y2 == 1 | X1_ik_vmmc_y1 == 1), na.rm = TRUE)) %>%
  # Count of all males who started study MC
  mutate(beganMC_count = sum(endpoint_coverage_mc == "Began study circumcised", na.rm = TRUE)) %>%
  ...
  mutate(hiv_began_infected_count = sum(hiv_status_current == "HIV-infected", na.rm = TRUE)) %>%
  # Count of those who refused HIV testing at baseline in each cluster
  mutate(hiv_refused_testing_count = sum(hiv_status_current == "Refused HIV testing", na.rm = TRUE)) %>%
  # Count of those on ART Year 1
  mutate(art_y1_count = sum(X3_ik_y1 == 1, na.rm = TRUE)) %>%
  # Count of those on ART Year 2
  mutate(art_y2_count = sum(X3_ik_y2 == 1, na.rm = TRUE)) %>%
  # Count of those on ART Year 3
  mutate(art_y3_count = sum(X3_ik_y3 == 1, na.rm = TRUE)) %>%
  # Count of those virally suppressed Year 1
  mutate(vlsupp_y1_count = sum(vlsupp_ik_y1 == 1, na.rm = TRUE)) %>%
  # Count of those virally suppressed Year 2
  mutate(vlsupp_y2_count = sum(vlsupp_ik_y2 == 1, na.rm = TRUE)) %>%
  # Count of those virally suppressed Year 3
  mutate(vlsupp_y3_count = sum(vlsupp_ik_y3 == 1, na.rm = TRUE)) %>%
  ungroup() %>%
  # Treatment related proportions
  mutate(Z1_k_baseline = beganMC_count/male_count, # Prop males MC at baseline
         Z1_k = mc_count/male_count, # Prop males circumcised at any point in study
         Z1_k_vmmc = vmmc_count/male_count, # Prop males VMMC at any point in study
         Z1_k_vmmc_y1 = vmmc_count_y1/male_count,
         Z1_k_vmmc_y2 = vmmc_count_y2/male_count,
         Z1_k_vmmc_y3 = vmmc_count_y3/male_count,
         Z2_k = htc_count/cluster_size,
         # On ART among those who are HIV+
         Z3_k_y1 = art_y1_count/hiv_began_infected_count,
         Z3_k_y2 = art_y2_count/hiv_began_infected_count,
         Z3_k_y3 = art_y3_count/hiv_began_infected_count) %>%
  # Characteristic related proportions
  mutate(male_prop = male_count/cluster_size,
         prop_vlsupp_y1 = vlsupp_y1_count/cluster_size,
         prop_vlsupp_y2 = vlsupp_y2_count/cluster_size,
         prop_vlsupp_y3 = vlsupp_y3_count/cluster_size,
         hiv_began_infected_prop = hiv_began_infected_count/cluster_size,
         hiv_refused_testing_prop = hiv_refused_testing_count/cluster_size) %>%
  mutate(Y_ik = ifelse(endpoint_seroconvert == "Yes", 1,
                       ifelse(endpoint_seroconvert == "No", 0, NA))) %>%
  # Cleaning Covariates
  mutate(# INDIVIDUAL
         baseline_age = ifelse(!is.na(age_at_interview_dat1), 
                               age_at_interview_dat1, 
                               ifelse(!is.na(age_at_interview_dat2), 
                                      age_at_interview_dat2 - 1,
                                      ifelse(!is.na(age_at_interview_dat3),
                                             age_at_interview_dat3 - 1, NA))),
         baseline_residence_length = ifelse(!is.na(length_residence_dat1), 
                                            length_residence_dat1,
                                            ifelse(!is.na(length_residence_dat2),
                                                   length_residence_dat2,
                                                   ifelse(!is.na(length_residence_dat3),
                                                          length_residence_dat3, NA))),
         baseline_marital_status = ifelse(!is.na(marital_status_dat1), 
                                 marital_status_dat1,
                                 ifelse(!is.na(marital_status_dat2),
                                        marital_status_dat2,
                                        ifelse(!is.na(marital_status_dat3),
                                               marital_status_dat3, NA))),
         
         baseline_education = ifelse(!is.na(education_dat1), 
                                 education_dat1,
                                 ifelse(!is.na(education_dat2),
                                        education_dat2,
                                        ifelse(!is.na(education_dat3),
                                               education_dat3, NA))),
         
         baseline_employment = ifelse(!is.na(employment_status_dat1), 
                                 employment_status_dat1,
                                 ifelse(!is.na(employment_status_dat2),
                                        employment_status_dat2,
                                        ifelse(!is.na(employment_status_dat3),
                                               employment_status_dat3, NA))),
         
         viral_load_y1 = Viral_load_dat1,
         
         viral_load_y2 = ifelse(!is.na(Viral_load_dat2), Viral_load_dat2,
                                ifelse(!is.na(viral_load_yr2_dat2), 
                                       viral_load_yr2_dat2, NA)),
         
         viral_load_y3 = ifelse(!is.na(Viral_load_dat3), Viral_load_dat3,
                                ifelse(!is.na(viral_load_yr3_dat3), 
                                       viral_load_yr3_dat3, NA)),
         
         arv_status_y1 = arv_status_current_dat1,
         
         arv_status_y2 = arv_status_current_dat2,
         
         arv_status_y3 = arv_status_current_dat3,
         
         # prioritize y3, get y2 or y1 data only if needed
         lifetime_partners = ifelse(!is.na(partners_lifetime_dat3), partners_lifetime_dat3,
                                    ifelse(!is.na(partners_lifetime_dat2), partners_lifetime_dat2,
                                           ifelse(!is.na(partners_lifetime_dat1), partners_lifetime_dat1, NA))),
         
         partners_12mos_y1 = case_when(partners_12mos_dat1 == "1 partner" ~ 1L,
                                       partners_12mos_dat1 == "2 partners" ~ 2L,
                                       partners_12mos_dat1 == "3 partners" ~ 3L,
                                       partners_12mos_dat1 == "4 or more partners" ~ 4L,
                                       partners_12mos_dat1 == "None" ~ 0L,
                                       TRUE ~ NA_integer_ 
                                       ),
         partners_12mos_y2 = case_when(partners_12mos_dat2 == "1 partner" ~ 1L,
                                       partners_12mos_dat2 == "2 partners" ~ 2L,
                                       partners_12mos_dat2 == "3 partners" ~ 3L,
                                       partners_12mos_dat2 == "4 or more partners" ~ 4L,
                                       partners_12mos_dat2 == "None" ~ 0L,
                                       TRUE ~ NA_integer_ 
                                       ),
         partners_12mos_y3 = case_when(partners_12mos_dat3 == "1 partner" ~ 1L,
                                       partners_12mos_dat3 == "2 partners" ~ 2L,
                                       partners_12mos_dat3 == "3 partners" ~ 3L,
                                       partners_12mos_dat3 == "4 or more partners" ~ 4L,
                                       partners_12mos_dat3 == "None" ~ 0L,
                                       TRUE ~ NA_integer_ 
                                       ),
         sex_12mos_y1 = sex_12mos_dat1,
         sex_12mos_y2 = sex_12mos_dat2,
         sex_12mos_y3 = sex_12mos_dat3,
         
         sex_outside_12mos_y1 = sex_outside_12mos_dat1,
         sex_outside_12mos_y2 = sex_outside_12mos_dat2,
         sex_outside_12mos_y3 = sex_outside_12mos_dat3,
         
         condom_lastsex_y1 = condom_lastsex_dat1,
         condom_lastsex_y2 = condom_lastsex_dat2,
         condom_lastsex_y3 = condom_lastsex_dat3,
         
         exchange_12mos_y1 = exchange_12mos_dat1,
         exchange_12mos_y2 = exchange_12mos_dat2,
         exchange_12mos_y3 = exchange_12mos_dat3,
         
         overall_access = overall_access_dat1,
         
         # CLUSTER
         prob_housing = ifelse(!is.na(prob_housing_dat1), prob_housing_dat2,
                               ifelse(!is.na(prob_housing_dat2), prob_housing_dat2, NA)),
         
         prob_schools = ifelse(!is.na(prob_schools_dat1), prob_schools_dat1,
                               ifelse(!is.na(prob_schools_dat2), prob_schools_dat2, NA)),
         
         prob_hiv = ifelse(!is.na(prob_hiv_dat1), prob_hiv_dat1, ifelse(!is.na(prob_hiv_dat2), prob_hiv_dat2, NA)),
         
         prob_alcohol_drug = ifelse(!is.na(prob_alcohol_drug_dat1), prob_alcohol_drug_dat1, 
                                    ifelse(!is.na(prob_alcohol_drug_dat2), prob_alcohol_drug_dat2, NA)),
         
         prob_healthcare = ifelse(!is.na(prob_healthcare_dat1), prob_healthcare_dat1, 
                                  ifelse(!is.na(prob_healthcare_dat2), prob_healthcare_dat2, NA))
         )

modelDat_full_final <- modelDat_full %>%
  dplyr::select(# Identification
                subject_id, subject_clus_id = subj_glmmter_id, cluster_id, 
                
                # Individual Characteristics
                gender, random_arm, T_k, hiv_status_current, 
                
                # MC
                X1_ik,
                
                # VMMC
                X1_ik_vmmc, # At any point
                X1_ik_vmmc_y1, X1_ik_vmmc_y2, X1_ik_vmmc_y3,
                
                # HTC
                X2_ik, # At any point (no data for year to year)
                
                # ART
                X3_ik_y1, X3_ik_y2, X3_ik_y3,
                
                # Viral load suppressed
                vlsupp_ik_y1, vlsupp_ik_y2, vlsupp_ik_y3,
                
                # HIV outcome
                endpoint_seroconvert, Y_ik,
                ...
                # Counts
                # Refused testing
                hiv_refused_testing_count, hiv_refused_testing_prop,
                # Began HIV infected
                hiv_began_infected_count, hiv_began_infected_prop,
                # Males in each cluster
                male_count, male_prop,
                # Males that began MC
                beganMC_count, Z1_k_baseline,
                # Males that were MC or VMMC at any point
                mc_count, Z1_k,
                # Males that were VMMC
                vmmc_count, Z1_k_vmmc,
                vmmc_count_y1, Z1_k_vmmc_y1,
                vmmc_count_y2, Z1_k_vmmc_y2,
                vmmc_count_y3, Z1_k_vmmc_y3,
                # HTC
                htc_count, Z2_k,
                # ART
                art_y1_count, Z3_k_y1,
                art_y2_count, Z3_k_y2,
                art_y3_count, Z3_k_y3,
                
                vlsupp_y1_count, prop_vlsupp_y1,
                vlsupp_y2_count, prop_vlsupp_y2,
                vlsupp_y3_count, prop_vlsupp_y3,
                
                # INDIVIDUAL COVARIATES
                baseline_age,
                baseline_residence_length,
                ...
                sex_outside_12mos_y1,
                sex_outside_12mos_y2,
                sex_outside_12mos_y3,
...
# Then filter out people who already have HIV or refused testing
modelDat_art <- modelDat_full_final %>% 
  dplyr::filter(hiv_status_current == "HIV-uninfected") %>%
  dplyr::filter(!is.na(Y_ik))

excludedDat_art <- modelDat_full_final %>%
  dplyr::filter(hiv_status_current != "HIV-uninfected" | is.na(hiv_status_current) | is.na(Y_ik))
```
...
# Missing data map
gg_miss_var(modelDat_full_final)
```



```{r, eval = FALSE, echo = FALSE}
tally(~endpoint_death, data = modelDat_full_final)
tally(~death_primary_cause, data = modelDat_full_final)
tally(~death_group_cause, data = modelDat_full_final)

tally(~endpoint_seroconvert, data = modelDat_full_final)

tally(~vlsupp_ik_y1, modelDat_full_final)

# On ART Y1
tally(endpoint_death ~ X3_ik_y1, data = modelDat_full_final) # Death outcome
tally(vlsupp_ik_y1 ~ X3_ik_y1, data = modelDat_full_final) # Viral load suppressed
histogram(~Z3_k_y1, data = modelDat_full_final)

# On ART Y2
tally(endpoint_death ~ X3_ik_y2, data = modelDat_full_final) # Death outcome
tally(vlsupp_ik_y2 ~ X3_ik_y2, data = modelDat_full_final) # Viral load suppressed
histogram(~Z3_k_y2, data = modelDat_full_final)

# On ART Y3
tally(endpoint_death ~ X3_ik_y3, data = modelDat_full_final) # Death outcome
tally(vlsupp_ik_y3 ~ X3_ik_y3, data = modelDat_full_final) # Viral load suppressed
histogram(~Z3_k_y3, data = modelDat_full_final)

ggplot(modelDat_full_final, aes(x = Z3_k_y1)) + facet_wrap(~endpoint_seroconvert) +
  geom_histogram(alpha = 0.5, position = "identity", bins = 30) +
  theme_minimal()

# VMMC Y1
tally(endpoint_seroconvert ~ X1_ik_vmmc_y1, data = modelDat_full_final)

# VMMC Y2
tally(endpoint_seroconvert ~ X1_ik_vmmc_y2, data = modelDat_full_final)

# VMMC Y3
tally(endpoint_seroconvert ~ X1_ik_vmmc_y3, data = modelDat_full_final)
```
...
dat <- dplyr::select(modelDat_full_final,
                     subject_id, cluster_id, Y_ik, Z3_k_y1, Z3_k_y2, Z3_k_y3)
# dat: subject_id, cluster_id, Y_ik, Z3_ik_y1, Z3_ik_y2, Z3_ik_y3

# As a distributed-lag style model: separate effects by year
# Treating each year's cluster adherence as separate predictor
gee_sep <- geeglm(
  Y_ik ~ Z3_k_y1 + Z3_k_y2 + Z3_k_y3,
  id = cluster_id, family = binomial, corstr = "exchangeable",
  data = dat
)
summary(gee_sep)

# As “level + trend”: mean adherence and changes year-to-year
dat2 <- transform(
  dat,
  mean_adherence = (Z3_k_y1 + Z3_k_y2 + Z3_k_y3)/3,
  change_2_vs_1  = Z3_k_y2 - Z3_k_y1,
  change_3_vs_2  = Z3_k_y3 - Z3_k_y2
)

gee_level_trend <- geeglm(
  Y_ik ~ mean_adherence + change_2_vs_1 + change_3_vs_2,
  id = cluster_id, family = binomial, corstr = "exchangeable",
  data = dat2
)

summary(gee_level_trend)

# Convert to odds ratios
or <- exp(coef(gee_level_trend))
cbind(OR = or)
...
glmm_sep <- glmer(
  Y_ik ~ Z3_k_y1 + Z3_k_y2 + Z3_k_y3 + (1 | cluster_id),
  family = binomial, data = dat
)
summary(glmm_sep)

# Level + trend version
glmm_level_trend <- glmer(
  Y_ik ~ mean_adherence + change_2_vs_1 + change_3_vs_2 + (1 | cluster_id),
  family = binomial, data = dat2
)
summary(glmm_level_trend)

# Odds ratios
exp(fixef(glmm_level_trend))
...
cluster_rates <- dat %>%
  group_by(cluster_id) %>%
  summarise(rate = mean(Y_ik == 1), .groups = "drop") %>%
  left_join(
    dat %>% distinct(cluster_id, Z3_k_y1, Z3_k_y2, Z3_k_y3),
    by = "cluster_id"
  ) %>%
  pivot_longer(starts_with("Z3_k_y"),
               names_to = "year", values_to = "prop_adherent") %>%
  mutate(year = factor(year,
                       levels = c("Z3_k_y1","Z3_k_y2","Z3_k_y3"),
                       labels = c("Year 1","Year 2","Year 3")))

ggplot(cluster_rates, aes(prop_adherent, rate)) +
  geom_point() +
  geom_smooth(se = TRUE) +
  facet_wrap(~ year, nrow = 1) +
  labs(x = "Cluster adherence (proportion)",
       y = "Cluster seroconversion rate",
       title = "Between-cluster seroconversion rate vs. adherence (by year)") +
  theme_bw()
...
```{r, eval = FALSE}
# Dist of ART proportion among HIV+ alone
as.tibble(rbind(favstats(~Z3_k_y1, data = modelDat_full_final),
                    favstats(~Z3_k_y2, data = modelDat_full_final))) %>%
  rbind(., favstats(~Z3_k_y3, data = modelDat_full_final)) %>%
  mutate(Year = c(1, 2, 3)) %>%
  dplyr::select(Year, Min = min, Mean = mean, Max = max, SD = sd)

# Dist of ART across outcome
as.tibble(rbind(favstats(Z3_k_y1 ~ Y_ik, data = modelDat_full_final), 
                favstats(Z3_k_y2 ~ Y_ik, data = modelDat_full_final))) %>%
  rbind(., favstats(Z3_k_y3 ~ Y_ik, data = modelDat_full_final)) %>%
  mutate(Year = c(1, 1, 2, 2, 3, 3)) %>%
  mutate(sd = round(sd, 3)) %>%
  dplyr::select(Year, Y_ik, Mean = mean, Min = min, Max = max, SD = sd, n = n) %>%
  arrange(Year, desc(Y_ik))

# Dist of ART across treatment assignment
as.tibble(rbind(favstats(Z3_k_y1 ~ random_arm, data = modelDat_full_final), 
                favstats(Z3_k_y2 ~ random_arm, data = modelDat_full_final))) %>%
  rbind(., favstats(Z3_k_y3 ~ random_arm, data = modelDat_full_final)) %>%
  mutate(Year = c(1, 1, 2, 2, 3, 3)) %>%
  mutate(sd = round(sd, 3)) %>%
  dplyr::select(Year, random_arm, Mean = mean, Min = min, Max = max, SD = sd, n = n) %>%
  arrange(Year, desc(random_arm))
```
...
```{r}
# Dist of ART across outcome
as.tibble(rbind(favstats(Z3_k_y1 ~ Y_ik, data = modelDat_SpW), 
                favstats(Z3_k_y2 ~ Y_ik, data = modelDat_SpW))) %>%
  rbind(., favstats(Z3_k_y3 ~ Y_ik, data = modelDat_SpW)) %>%
  mutate(Year = c(1, 1, 2, 2, 3, 3)) %>%
  mutate(sd = round(sd, 3)) %>%
  dplyr::select(Year, Y_ik, Mean = mean, Min = min, Max = max, SD = sd, n = n) %>%
  arrange(Year, desc(Y_ik))
...
# Dist of ART across treatment assignment
as.tibble(rbind(favstats(Z3_k_y1 ~ random_arm, data = modelDat_SpW), 
                favstats(Z3_k_y2 ~ random_arm, data = modelDat_SpW))) %>%
  rbind(., favstats(Z3_k_y3 ~ random_arm, data = modelDat_SpW)) %>%
  mutate(Year = c(1, 1, 2, 2, 3, 3)) %>%
  mutate(sd = round(sd, 3)) %>%
  dplyr::select(Year, random_arm, Mean = mean, Min = min, Max = max, SD = sd, n = n) %>%
  arrange(Year, desc(random_arm))
...
Then, under certain assumptions, the only way for an intervention village participant to have lower HIV risk is by association with others in the village with lower HIV risk because of their exposure to the intervention. We control for cluster proportions of men with VMMC, proportion who adhered to HTC, and proportion HIV+ ($Z^{(1)}_k$, $Z^{(2)}_k$, Prop_HIV). 
...
# Model not accounting for clustering
model_SpW <- glm(Y_ik ~ T_k + hiv_began_infected_prop,
                 family = binomial(link = 'logit'),
                 data = modelDat_SpW) # Exclude those who received full trt

# Model accounting for clustering using GLMM
model_SpW_glmm <- glmer(Y_ik ~ T_k + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable structure
                        data = modelDat_SpW, 
                        family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_SpW_gee <- geeglm(Y_ik ~ T_k + hiv_began_infected_prop,
...
exp_beta_SpW_2 <- exp(model_SpW_summary$coefficients[3,1]) # Prop HIV+ Coefficient
...
exp_beta_SpW_2_glmm <- exp(model_SpW_summary_glmm$coefficients[3,1]) # Prop HIV+ Coefficient
...
exp_beta_SpW_2_gee <- exp(model_SpW_summary_gee$coefficients[3,1]) # Prop HIV+ Coefficient
...
# Model not accounting for clustering
model_SpWR <- glm(Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop,
                  family = binomial(link = 'logit'),
                  data = modelDat_SpW)

# Model accounting for clustering using GLMM
model_SpWR_glmm <- glmer(Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable
                         data = modelDat_SpW, 
                         family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_SpWR_gee <- geeglm(Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop,
                         family = binomial(link = "logit"),
                         id = cluster_id,
                         data = modelDat_SpW,
                         corstr = "exchangeable") # working correlation
...
exp_beta_SpWR_2 <- exp(model_SpWR_summary$coefficients[3,1]) # Z3_y3 Coefficient
exp_beta_SpWR_3 <- exp(model_SpWR_summary$coefficients[4,1]) # Prop HIV+ Coefficient
...
exp_beta_SpWR_2_glmm <- exp(model_SpWR_summary_glmm$coefficients[3,1]) # Z3_y3 Coefficient
exp_beta_SpWR_3_glmm <- exp(model_SpWR_summary_glmm$coefficients[4,1]) # Prop HIV+ Coefficient
...
tidy_SpWR_glmm <- broom.mixed::tidy(model_SpWR_glmm, effects = "fixed", 
  ...
  dplyr::select(Model, Term, `OR [95% CI]`, `p-value`) %>%
  dplyr::filter(Term != "(Intercept)") %>%
  mutate(ICC = c(round(performance::icc(model_SpW_glmm)$ICC_adjusted[[1]], 3), NA, NA))
...
exp_beta_SpWR_2_gee <- exp(model_SpWR_summary_gee$coefficients[3,1]) # Z3_y3 Coefficient
exp_beta_SpWR_3_gee <- exp(model_SpWR_summary_gee$coefficients[4,1]) # Prop HIV+ Coefficient
...
tidy_SpWR_gee <- broom::tidy(model_SpWR_gee, conf.int = TRUE, 
  ...
  dplyr::select(Model, Term, `OR [95% CI]`, `p-value`) %>%
  dplyr::filter(Term != "(Intercept)") %>%
  mutate(ICC = c(round(model_SpWR_gee$geese$alpha[[1]], 3), NA, NA))
...
```{r}
# ------------------------------------------------------------
# 95% CI for the INDIRECT effect (difference method) in GLM
#   IE_logit = [beta_T from Y ~ T + covars]  -  [beta_T from Y ~ T + Z + covars]
#
# * Same sample & same covariates in both models (only Z differs).
# * Optionally cluster-bootstrap by 'cluster_id'; else row bootstrap.
# * Returns CI on log-odds and OR scales.
# ------------------------------------------------------------
ie_ci_diff_glm <- function(data, y, t, z, covars = NULL,
                           B = 500, level = 0.95,
                           cluster_id = NULL,
  ...
  # --- columns needed
  need <- unique(c(y, t, z, covars, if (!is.null(cluster_id)) cluster_id))
  miss <- setdiff(need, names(data))
  if (length(miss)) stop("Missing columns: ", paste(miss, collapse = ", "))

  # --- complete cases across all used variables (same sample for both fits)
  dat <- data[stats::complete.cases(data[, need, drop = FALSE]), , drop = FALSE]
  if (nrow(dat) == 0) stop("No complete cases after filtering on required variables.")

  # --- formulas
  rhs_total <- paste(c(t, covars), collapse = " + ")
  rhs_dir   <- paste(c(t, z, covars), collapse = " + ")
  f_total   <- stats::as.formula(paste(y, "~", rhs_total))
  f_direct  <- stats::as.formula(paste(y, "~", rhs_dir))

  ctrl <- stats::glm.control(maxit = 50)

  # --- full-data fits
  fit_t <- stats::glm(f_total,  family = stats::binomial(), data = dat, control = ctrl)
  fit_d <- stats::glm(f_direct, family = stats::binomial(), data = dat, control = ctrl)

  # NOTE: assumes 't' is a single coefficient (e.g., 0/1). If t is a factor with >2 levels,
  #       you’ll need to adapt which contrast you extract.
  a0 <- unname(stats::coef(fit_t)[t])   # beta_T from Y ~ T + covars
  c0 <- unname(stats::coef(fit_d)[t])   # beta_T from Y ~ T + Z + covars
  if (any(is.na(c(a0, c0)))) stop("Could not extract the T coefficient. Check that 't' is coded as a single term.")

  ie0_logit <- a0 - c0
  ie0_or    <- exp(ie0_logit)

  # --- bootstrap draws of (a, c)
  ...
  for (b in seq_len(B)) {
    if (is.null(cluster_id)) {
      ...
      # Cluster bootstrap
      cl <- unique(dat[[cluster_id]])
      samp <- sample(cl, length(cl), replace = TRUE)
      boot_dat <- do.call(rbind, lapply(samp, function(k) dat[dat[[cluster_id]] == k, , drop = FALSE]))
    }

    bt <- try(stats::glm(f_total,  family = stats::binomial(), data = boot_dat, control = ctrl), silent = TRUE)
    bd <- try(stats::glm(f_direct, family = stats::binomial(), data = boot_dat, control = ctrl), silent = TRUE)

    if (!inherits(bt, "try-error") && !inherits(bd, "try-error")) {
      a <- suppressWarnings(unname(stats::coef(bt)[t]))
      c <- suppressWarnings(unname(stats::coef(bd)[t]))
      if (all(is.finite(c(a, c)))) theta[b, ] <- c(a = a, c = c)
    }
    if (verbose && b %% msg_every == 0) message("bootstrap: ", b, "/", B)
  }

  # --- joint covariance from bootstrap & CI for (a - c)
  Sigma <- stats::cov(theta, use = "pairwise.complete.obs")
  var_diff <- Sigma[1,1] + Sigma[2,2] - 2 * Sigma[1,2]
  se_diff  <- sqrt(var_diff)

  z <- stats::qnorm(1 - (1 - level)/2)
  ci_logit <- ie0_logit + c(-1, 1) * z * se_diff
  ci_or    <- exp(ci_logit)

  list(
    estimate_logit = ie0_logit,
    ci_logit       = ci_logit,
    estimate_or    = ie0_or,
    ci_or          = ci_or,
    draws_used     = sum(stats::complete.cases(theta)),
    settings       = list(B = B, level = level, covars = covars,
                          cluster_id = cluster_id, n = nrow(dat))
  )
}

# ------------------------------------------------------------
# 95% CI for the INDIRECT effect (difference method) in GLMM
#   IE_logit = [beta_T from Y ~ T + covars + (1|cluster)] 
#            - [beta_T from Y ~ T + Z + covars + (1|cluster)]
#
# * Same complete-case sample and covariates in both models (only Z differs).
# * Cluster bootstrap to get joint Cov(a, c); duplicates get a new .boot_id RE.
# * Returns CI on log-odds and OR scales.
# ------------------------------------------------------------
ie_ci_diff_glmm <- function(data, y, t, z, covars = NULL,
                            cluster_id,
                            B = 500, level = 0.95,
                            seed = NULL, verbose = FALSE,
  ...
  if (!is.null(seed)) set.seed(seed)
  stopifnot(is.data.frame(data))
  if (missing(cluster_id)) stop("Please provide 'cluster_id' (random-effect grouping variable).")

  # --- columns needed & complete-case filter (same sample in both fits)
  need <- unique(c(y, t, z, covars, cluster_id))
  miss <- setdiff(need, names(data))
  if (length(miss)) stop("Missing columns: ", paste(miss, collapse = ", "))
  ...
  rhs_total <- paste(c(t, covars), collapse = " + ")
  rhs_dir   <- paste(c(t, z, covars), collapse = " + ")
  f_total_full  <- stats::as.formula(paste0(y, " ~ ", rhs_total, " + (1|", cluster_id, ")"))
  f_direct_full <- stats::as.formula(paste0(y, " ~ ", rhs_dir,   " + (1|", cluster_id, ")"))

  # --- glmer control: help convergence with sparse events
  ctrl <- lme4::glmerControl(optimizer = "bobyqa",
                             optCtrl   = list(maxfun = 2e5),
                             check.conv.grad = "ignore",
                             check.conv.singular = "ignore")

  # --- full-data fits & T-coefficients
  fit_total  <- suppressWarnings(lme4::glmer(f_total_full,  family = stats::binomial(), data = dat, control = ctrl))
  fit_direct <- suppressWarnings(lme4::glmer(f_direct_full, family = stats::binomial(), data = dat, control = ctrl))

  a0 <- as.numeric(lme4::fixef(fit_total )[t])  # beta_T (total)
  c0 <- as.numeric(lme4::fixef(fit_direct)[t])  # beta_T (direct)
  if (any(is.na(c(a0, c0)))) stop("Could not extract the T coefficient; ensure 't' is a single term.")

  ie0_logit <- a0 - c0
  ie0_or    <- exp(ie0_logit)

  # --- cluster strata (by treatment) & event flags: stabilize boot samples
  #     (assumes binary T; if not, you can drop stratification)
  cl_T <- tapply(dat[[t]], dat[[cluster_id]], function(x) {
    ux <- unique(x); if (length(ux) == 1) ux else as.numeric(mean(x, na.rm = TRUE) >= 0.5)
  })
  cl_event <- tapply(dat[[y]], dat[[cluster_id]], function(v) any(v == 1, na.rm = TRUE))
  ...
  # --- helper: build one bootstrap dataset; duplicates get new .boot_id
  make_boot_df <- function() {
    ...
    for (j in seq_along(samp_ids)) {
      k <- samp_ids[j]
      d <- dat[dat[[cluster_id]] == k, , drop = FALSE]
  ...
  # boot formulas: use .boot_id as the grouping factor
  f_total_boot  <- stats::as.formula("Y ~ T + (1|.boot_id)")
  f_direct_boot <- stats::as.formula("Y ~ T + Z + (1|.boot_id)")

  # map generic names to actual column names for model.frame
  # (we'll construct a temp data.frame with standardized column names)
  for (b in seq_len(B)) {
    bd <- make_boot_df()
    # standardize names expected by boot formulas
    bd$Y <- bd[[y]]
    bd$T <- bd[[t]]
    bd$Z <- bd[[z]]
    # include covariates (keep their actual names; they’re referenced via data=bd)
    # build the final boot formulas including covariates
    if (length(covars)) {
      rhs_cov <- paste(covars, collapse = " + ")
      f_total_boot  <- stats::as.formula(paste0("Y ~ T + ", rhs_cov, " + (1|.boot_id)"))
      f_direct_boot <- stats::as.formula(paste0("Y ~ T + Z + ", rhs_cov, " + (1|.boot_id)"))
    }

    fit_t <- try(suppressWarnings(
      lme4::glmer(f_total_boot,  family = stats::binomial(), data = bd, control = ctrl)
    ), silent = TRUE)

    fit_d <- try(suppressWarnings(
      lme4::glmer(f_direct_boot, family = stats::binomial(), data = bd, control = ctrl)
    ), silent = TRUE)
  ...
  # --- joint covariance of (a, c) from bootstrap
  Sigma <- stats::cov(theta, use = "pairwise.complete.obs")
  var_diff <- Sigma[1,1] + Sigma[2,2] - 2 * Sigma[1,2]
  se_diff  <- sqrt(var_diff)
  ...
  list(
    ...
    draws_used     = sum(stats::complete.cases(theta)),
    settings       = list(
      B = B, level = level, covars = covars, cluster_id = cluster_id,
      force_event_once = force_event_once, allow_dup_events = allow_dup_events
    )
  )
}

# ------------------------------------------------------------
# INDIRECT effect (difference method) via GEE with covariates
#   IE_logit = [beta_T from Y ~ T + covars]  -  [beta_T from Y ~ T + Z + covars]
#
# * Same complete-case sample & covariates in both models (only Z differs).
# * Cluster bootstrap to estimate joint Cov(a, c); duplicates get new .boot_id.
# * CI for (a - c) uses Var(a-c)=Var(a)+Var(c)-2Cov(a,c); OR CI by exponentiating.
# ------------------------------------------------------------
ie_ci_diff_gee <- function(data, y, t, z, covars = NULL,
                           cluster_id,
                           B = 350, level = 0.95,
                           corstr = "exchangeable",
                           seed = NULL, verbose = FALSE,
  ...
  if (!is.null(seed)) set.seed(seed)
  stopifnot(is.data.frame(data))
  if (missing(cluster_id)) stop("Please provide 'cluster_id' (GEE id variable).")

  # --- columns & complete-case filter (same sample in both fits)
  need <- unique(c(y, t, z, covars, cluster_id))
  miss <- setdiff(need, names(data))
  if (length(miss)) stop("Missing columns: ", paste(miss, collapse = ", "))
  ...
  id_full <- dat[[cluster_id]]
  fit_t <- suppressWarnings(geepack::geeglm(f_total,  id = id_full, data = dat,
                                            family = stats::binomial(), corstr = corstr))
  fit_d <- suppressWarnings(geepack::geeglm(f_direct, id = id_full, data = dat,
                                            family = stats::binomial(), corstr = corstr))

  # NOTE: assumes `t` is a single 0/1 term. If `t` is a multi-level factor,
  #       choose the contrast to extract (adapt the coef name accordingly).
  a0 <- suppressWarnings(unname(stats::coef(fit_t)[t]))  # total model T effect
  c0 <- suppressWarnings(unname(stats::coef(fit_d)[t]))  # direct model T effect
  if (any(is.na(c(a0, c0)))) stop("Could not extract the T coefficient; is 't' a single term?")

  ie0_logit <- a0 - c0
  ie0_or    <- exp(ie0_logit)

  # --- cluster strata by treatment & event flags (stabilize bootstrap)
  cl_T <- tapply(dat[[t]], dat[[cluster_id]], function(x) {
    ux <- unique(x); if (length(ux) == 1) ux else as.numeric(mean(x, na.rm = TRUE) >= 0.5)
  })
  cl_event <- tapply(dat[[y]], dat[[cluster_id]], function(v) any(v == 1, na.rm = TRUE))
  ...
  # --- build one bootstrap dataset; duplicates get new .boot_id for GEE
  make_boot_df <- function() {
    base    <- if (force_event_once) c(evt_t1, evt_t0) else character(0)
    fill_t1 <- if (n1 - length(evt_t1) > 0) sample(pool_t1, n1 - length(evt_t1), replace = TRUE) else character(0)
    fill_t0 <- if (n0 - length(evt_t0) > 0) sample(pool_t0, n0 - length(evt_t0), replace = TRUE) else character(0)
  ...
  for (b in seq_len(B)) {
    ...
    bt <- try(suppressWarnings(
      geepack::geeglm(f_total,  id = idB, data = bd, family = stats::binomial(), corstr = corstr)
    ), silent = TRUE)

    bd_fit <- try(suppressWarnings(
      geepack::geeglm(f_direct, id = idB, data = bd, family = stats::binomial(), corstr = corstr)
    ), silent = TRUE)

    if (!inherits(bt, "try-error") && !inherits(bd_fit, "try-error")) {
      a <- suppressWarnings(unname(stats::coef(bt)[t]))
      c <- suppressWarnings(unname(stats::coef(bd_fit)[t]))
      if (all(is.finite(c(a, c)))) theta[b, ] <- c(a = a, c = c)
    }
  ...
  list(
    ...
    draws_used     = sum(stats::complete.cases(theta)),
    settings       = list(
      B = B, level = level, corstr = corstr, covars = covars,
      cluster_id = cluster_id, force_event_once = force_event_once,
      allow_dup_events = allow_dup_events, n = nrow(dat)
    )
  )
}
...
```{r}
# Indirect (difference) CI for spillover with HTC, adjusting for Z_other and age as covariates:
res <- ie_ci_diff_glm(
  data       = modelDat_SpW,
  y          = "Y_ik",
  t          = "T_k",
  z          = "Z3_k_y3",      # mediator (spillover proportion), rename as needed
  covars     = c("hiv_began_infected_prop"),
  cluster_id = "cluster_id",     # cluster bootstrap (omit for row bootstrap)
  B = 600, level = 0.95, seed = 123, verbose = TRUE
)

res$estimate_logit; res$ci_logit
res$estimate_or;    res$ci_or


# Example: spillover HTC indirect effect with covariates (GLMM)
res_glmm <- ie_ci_diff_glmm(
  data       = modelDat_SpW,
  y          = "Y_ik",
  t          = "T_k",
  z          = "Z3_k_y3",      # mediator (village HTC proportion)
  covars     = c("hiv_began_infected_prop"),  # example covariates
  cluster_id = "cluster_id",
  B = 400, level = 0.95, seed = 123, verbose = TRUE
)

res_glmm$estimate_logit; res_glmm$ci_logit
res_glmm$estimate_or;    res_glmm$ci_or


# Example: HTC spillover, adjusting for covariates
res_gee <- ie_ci_diff_gee(
  data       = modelDat_SpW,
  y          = "Y_ik",
  t          = "T_k",
  z          = "Z3_k_y3",            # mediator (village HTC proportion)
  covars     = c("hiv_began_infected_prop"),   # optional
  cluster_id = "cluster_id",
  B = 200, level = 0.95, corstr = "exchangeable",
  seed = 123, verbose = TRUE
)

res_gee$estimate_logit; res_gee$ci_logit
res_gee$estimate_or;    res_gee$ci_or
...
pm_spill_boot_with_delta_glm <- function(dat, id = "cluster_id",
  ...
  req <- c(id, "Y_ik", "T_k", "Z3_k_y3", "hiv_began_infected_prop")
  miss <- setdiff(req, names(dat))
  if (length(miss)) stop("Missing columns: ", paste(miss, collapse = ", "))

  # --- formulas
  f_SpW  <- stats::as.formula("Y_ik ~ T_k + hiv_began_infected_prop")
  f_SpWR <- stats::as.formula("Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop")
...
pm_spill_boot_with_delta_glmm <- function(dat, id = "cluster_id",
  ...
  # --- Full-data fits -> point estimates
  fW_full  <- stats::as.formula(paste0("Y_ik ~ T_k + hiv_began_infected_prop + (1|", id, ")"))
  fWR_full <- stats::as.formula(paste0("Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop + (1|", id, ")"))
  ...
  fW_boot  <- stats::as.formula("Y_ik ~ T_k + hiv_began_infected_prop + (1|.boot_id)")
  fWR_boot <- stats::as.formula("Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop + (1|.boot_id)")
...
pm_spill_boot_with_delta_gee <- function(dat, id = "cluster_id",
  ...
  stopifnot(is.data.frame(dat))
  req <- c(id, "Y_ik", "T_k", "hiv_began_infected_prop", "Z3_k_y3")
  miss <- setdiff(req, names(dat))
  if (length(miss)) stop("Missing columns: ", paste(miss, collapse = ", "))

  # --- Formulas
  fW  <- stats::as.formula("Y_ik ~ T_k + hiv_began_infected_prop")                 # SpW
  fWR <- stats::as.formula("Y_ik ~ T_k + Z3_k_y3 + hiv_began_infected_prop")     # SpWR
...
modelDat_Ind <- modelDat_full_final %>%
  dplyr::filter(!is.na(vlsupp_ik_y3))
nrow(modelDat_full_final) - nrow(modelDat_Ind)
nrow(modelDat_Ind)

tally(X3_ik_y3~vlsupp_ik_y3, data = modelDat_Ind)

tally(X3_ik_y3~vlsupp_ik_y3, data = modelDat_full_final)
```
...
# Model not accounting for clustering
model_Ind <- glm(vlsupp_ik_y3 ~ T_k + hiv_began_infected_prop,
                 family = binomial(link = 'logit'),
                 data = modelDat_Ind)

# Model accounting for clustering using GLMM
model_Ind_glmm <- glmer(vlsupp_ik_y3 ~ T_k + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable
                        data = modelDat_Ind,
                        family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_Ind_gee <- geeglm(vlsupp_ik_y3 ~ T_k + hiv_began_infected_prop,
                        family = binomial(link = "logit"),
                        id = cluster_id,
                        data = modelDat_Ind,
                        corstr = "exchangeable") # working correlation
...
# Model not accounting for clustering
model_IndD <- glm(vlsupp_ik_y3 ~ T_k + X3_ik_y3 + hiv_began_infected_prop,
                  family = binomial(link = 'logit'),
                  data = modelDat_Ind)

# Model accounting for clustering using GLMM
model_IndD_glmm <- glmer(vlsupp_ik_y3 ~ T_k + X3_ik_y3 + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable
                         data = modelDat_Ind,
                         family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_IndD_gee <- geeglm(vlsupp_ik_y3 ~ T_k + X3_ik_y3 + hiv_began_infected_prop,
                         family = binomial(link = "logit"),
                         id = cluster_id,
                         data = modelDat_Ind,
                         corstr = "exchangeable") # working correlation
...
exp_beta_IndD_2 <- exp(model_IndD_summary$coefficients[3,1]) # X3_ik_y3 Coefficient
...
exp_beta_IndD_2_glmm <- exp(model_IndD_summary_glmm$coefficients[3,1]) # X3_ik_y3 Coefficient
...
exp_beta_IndD_2_gee <- exp(model_IndD_summary_gee$coefficients[3,1]) # X3_ik_y3 Coefficient
