...
```{r, eval = FALSE}
htc_order <- c("Yes", "No", "Missing")

htc_table <- as_tibble(tally(~endpoint_coverage_htc, data = modelDat)) %>%
  mutate(HTC = factor(endpoint_coverage_htc, levels = htc_order)) %>%
  dplyr::select(HTC, n) %>%
  arrange(HTC)

htc_hiv_table <- modelDat %>%
  dplyr::select(endpoint_seroconvert, endpoint_coverage_htc) %>%
  group_by(endpoint_seroconvert, endpoint_coverage_htc) %>%
  dplyr::summarize(n = n()) %>%
  pivot_wider(names_from = endpoint_seroconvert, values_from = n) %>%
  mutate(HTC = factor(endpoint_coverage_htc, levels = htc_order),
         `HIV No` = No, `HIV Yes` = Yes) %>%
  dplyr::select(HTC, `HIV Yes`, `HIV No`) %>%
  arrange(HTC)
```

```{r}
tally(endpoint_seroconvert ~ random_arm, data = modelDat_full)
```


## Crude analysis on HTC and HIV

Fisher's 2x2 Exact Test

```{r}
tab <- with(modelDat, table(Y_ik, X2_ik))
tab
...
set.seed(123)
fit_ex <- elrm(formula  = y / n ~ T_k + X2_ik,
               interest = ~ X2_ik,          # parameter of interest
...
### C. Within-Cluster Spillover of the Intervention Effect Not through HTC

"SpWR" denotes all the remaining spillover that affects one's outcome that exists when we block the mediated spillover path that exists through HTC.

$$logit(Y_{ik}) = \beta_0^{\text{SpWR}} + \beta_1^{\text{SpWR}}(T_{k}) + \beta_2^{\text{SpWR}}(Z_k^{(2)})$$

Here, $\exp\left(\beta_1^{\text{SpWR}} \right)$ compares untreated individuals in CP villages with untreated individuals in control villages after we hold the village's HTC coverage fixed at the same value for both groups. So, it's the OR for the remaining within-village spillover - whatever protection (or risk) is left once the HTC pathway has been accounted for. 
...
# Model not accounting for clustering
model_SpWR <- glm(Y_ik ~ T_k + Z2_k,
                  family = binomial(link = 'logit'),
                  data = modelDat_SpW)

# Model accounting for clustering using GLMM
model_SpWR_glmm <- glmer(Y_ik ~ T_k + Z2_k + (1|cluster_id), # Uses exchangeable
                         data = modelDat_SpW, 
                         family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_SpWR_gee <- geeglm(Y_ik ~ T_k + Z2_k,
                         family = binomial(link = "logit"),
                         id = cluster_id,
                         data = modelDat_SpW,
                         corstr = "exchangeable") # working correlation
...
print(xtable(SpWR_Model_Table, 
             caption = "Spillover Not Due to HTC Model Output"), 
      type = 'latex', include.rownames = FALSE, comment = FALSE)
```
