...
modelDat_Ind <- modelDat_full_final %>%
  dplyr::filter(!is.na(vlsupp_ik_y1))
nrow(modelDat_full_final) - nrow(modelDat_Ind)
nrow(modelDat_Ind)

tally(X3_ik_y1~vlsupp_ik_y1, data = modelDat_Ind)

tally(X3_ik_y1~vlsupp_ik_y1, data = modelDat_full_final)
```
...
model_Ind <- glm(vlsupp_ik_y1 ~ T_k + hiv_began_infected_prop,
                 family = binomial(link = 'logit'),
                 data = modelDat_Ind)

# Model accounting for clustering using GLMM
model_Ind_glmm <- glmer(vlsupp_ik_y1 ~ T_k + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable
                        data = modelDat_Ind,
                        family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_Ind_gee <- geeglm(vlsupp_ik_y1 ~ T_k + hiv_began_infected_prop,
                        family = binomial(link = "logit"),
                        id = cluster_id,
                        data = modelDat_Ind,
...
# Model not accounting for clustering
model_IndD <- glm(vlsupp_ik_y1 ~ T_k + X3_ik_y1 + hiv_began_infected_prop,
                  family = binomial(link = 'logit'),
                  data = modelDat_Ind)

# Model accounting for clustering using GLMM
model_IndD_glmm <- glmer(vlsupp_ik_y1 ~ T_k + X3_ik_y1 + hiv_began_infected_prop + (1|cluster_id), # Uses exchangeable
                         data = modelDat_Ind,
                         family = binomial(link = "logit"))

# Model accounting for clustering using GEE
model_IndD_gee <- geeglm(vlsupp_ik_y1 ~ T_k + X3_ik_y1 + hiv_began_infected_prop,
                         family = binomial(link = "logit"),
                         id = cluster_id,
                         data = modelDat_Ind,
                         corstr = "exchangeable") # working correlation
...
exp_beta_IndD_2 <- exp(model_IndD_summary$coefficients[3,1]) # X3_ik_y1 Coefficient
...
exp_beta_IndD_2_glmm <- exp(model_IndD_summary_glmm$coefficients[3,1]) # X3_ik_y1 Coefficient
...
exp_beta_IndD_2_gee <- exp(model_IndD_summary_gee$coefficients[3,1]) # X3_ik_y1 Coefficient
