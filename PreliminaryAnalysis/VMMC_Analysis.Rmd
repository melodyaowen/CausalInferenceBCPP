...
```{r echo = FALSE}
vmmcDat <- modelDat %>%
  mutate(VMMC = ifelse(endpoint_coverage_mc == "Yes", "Yes", 
...
```{r}
tally(Y_ik ~ VMMC, data = vmmcDat) # For all individuals
tally(Y_ik ~ VMMC, data = filter(vmmcDat, gender == "Male"))
tally(Y_ik ~ gender, data = vmmcDat)
```
...
model_y_vmmc <- glm(Y_ik ~ VMMC,
                    ...
                    data = filter(vmmcDat, gender == "Male"))
...
model_y_vmmc_gee <- geeglm(Y_ik ~ VMMC,
                           ...
                           data = filter(vmmcDat, gender == "Male", !is.na(VMMC)),
