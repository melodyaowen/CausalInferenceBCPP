...
The mediator model regresses the mediator on the exposure and confounders. Here, we block the spillover that exists through the proportion circumcised and proportion who received HTC in the cluster by controlling for it in the model. The abbreviation "IndM" refers to the individual effect mediator model, shown below.

$$logit(X_{ik}^{(1)}) = \beta_0^{\text{IndM}} + \beta_1^{\text{IndM}}(T_{k})$$
...
fits_IndM_glmm <- lapply(imp_list, function(d)
  glmer(X1_ik ~ T_k + (1|cluster_id), # Uses exchangeable
                         data = d, 
                         family = binomial(link = "logit")))
...
pooled_results_IndM_glmm <- pool_rubins(fits_IndM_glmm, 
  ...
  relocate(Model) %>%
  filter(Term != "(Intercept)") %>%
  mutate(`Mean ICC` = c(icc_IndM_glmm_mean))
...
pooled_results_IndM_gee <- pool_rubins(fits_IndM_gee,
  ...
  relocate(Model) %>%
  filter(Term != "(Intercept)") %>%
  mutate(`Mean ICC` = c(alpha_mean_IndM_gee))
...
fits_IndO_glmm <- lapply(imp_list, function(d)
  glmer(Y_ik ~ T_k + X1_ik + (1|cluster_id), # Uses exchangeable
                         data = d, 
                         family = binomial(link = "logit")))
...
pooled_results_IndO_glmm <- pool_rubins(fits_IndO_glmm, 
  ...
  relocate(Model) %>%
  filter(Term != "(Intercept)") %>%
  mutate(`Mean ICC` = c(icc_IndO_glmm_mean, NA))
...
pooled_results_IndO_gee <- pool_rubins(fits_IndO_gee,
  ...
  relocate(Model) %>%
  filter(Term != "(Intercept)") %>%
  mutate(`Mean ICC` = c(alpha_mean_IndO_gee, NA))
